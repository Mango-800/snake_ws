...
class SnakeServerNode(Node):
    def __init__(self):
        ...
        self.create_timer(0.1, self.zigzag_movement)

        self.zigzag_enabled = False
        self.zigzag_phase = 0.0
        self.zigzag_amplitude = 0.5  # Ajusta la amplitud del zigzag
        self.zigzag_frequency = 2.0 # Ajusta la frecuencia del zigzag
    ...
    def zigzag_movement(self):
        if self.zigzag_enabled:
            self.zigzag_phase += 0.1  # Incrementa la fase
            yaw_value = self.zigzag_amplitude * math.sin(self.zigzag_frequency * self.zigzag_phase)
            self.auxModule.angular.z = yaw_value
            msg_to_publish = Twist()
            msg_to_publish.linear.x = self.auxModule.linear.x
            msg_to_publish.angular.z = self.auxModule.angular.z
            self.module_master_publisher.publish(msg_to_publish)
